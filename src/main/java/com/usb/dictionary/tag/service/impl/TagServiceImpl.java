    ...
    public SaveTagServiceResponse save(SaveTagServiceRequest saveTagServiceRequest){
        ...
        if(!tagByNameOptional.isPresent()) {
            Tag newTag = Tag.builder()
                    ...
                    .name(saveTagServiceRequest.getName()).build();
    ...
    private void saveTagsToFullTextSearchRepository(String stringEntryModified) {
        try {
            ...
            if(!CollectionUtils.isEmpty(entryModified.getTags())){
                entryModified.getTags().forEach(tag->this.save(SaveTagServiceRequest.builder().name(tag).build()));
            }
        }
        catch (Exception exception){
            log.info("message=\"an error occurred\"" +
                    ", method=saveTagsToFullTextSearchRepository, feature=TagServiceImpl", exception);
        }
    }
}
